...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchProfile = useCallback(async (userId: string) => {
    ...
    try {
      const { data, error } = await supabase
        .from("profiles")
        .select(
          "id, full_name, email, avatar_url, role, beta_features, account_id, account_role, menu_access",
        )
        .eq("user_id", userId)
        .maybeSingle();
      ...
      if (data) {
        ...
        if (data.account_id) {
          const { data: account, error: accountErr } = await supabase
            .from("accounts")
            .select("id, name, default_currency, license_key, plan, license_activated_at, license_expires_at, allowed_menus")
          ...
          if (accountErr) {
            ...
            accountRow = {
              ...
              license_activated_at: account.license_activated_at ?? null,
              license_expires_at: account.license_expires_at ?? null,
              allowed_menus: account.allowed_menus ?? null,
            };
          }
        }
  ...
  const signOut = useCallback(async () => {
    // Send logout alert email (non-blocking)
    fetch("/api/auth/logout-alert", { method: "POST" }).catch(() => {});
    const supabase = createClient();
